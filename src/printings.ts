...
const VICTORY_EFFECT_DURATION_MS = 1500;
...
function renderCondensedTimeline() {
  ...
  for (const setCode of codesToRender) {
    const set = allSets.find(item => item.code === setCode);
    if (!set) {
      continue;
    }
    const item = document.createElement("div");
...
function revealVictory(printing: PrintingInfo, finish: Finish, message?: string) {
...
function showStoredDailyVictory(record: DailyPlayRecord) {
  ...
  if (correctPrinting) {
    ...
    revealVictory(correctPrinting, correctFinish, message);
  }
  shareResultsButton.classList.remove("hidden");
}
