...
const giveUpButton = document.getElementById("giveUpButton") as HTMLButtonElement;
...
const postWinActions = document.getElementById("postWinActions") as HTMLElement;
...
const allPrintingsSection = document.getElementById("allPrintingsSection") as HTMLElement;
const allPrintingsDetails = document.getElementById("allPrintingsDetails") as HTMLDetailsElement;
const allPrintingsGrid = document.getElementById("allPrintingsGrid") as HTMLElement;
...
const victoryOverlay = document.getElementById("victoryOverlay") as HTMLElement;
const victoryConfetti = document.getElementById("victoryConfetti") as HTMLElement;
...
let guessedSetCodesInOrder: string[] = [];
...
(function initTimelineDragScroll() {
  ...
  setTimeline.addEventListener("mousedown", (e: MouseEvent) => {
    ...
    if (setTimeline.classList.contains("set-timeline-condensed")) return;
    isDragging = true;
...
const SHARE_URL = "https://www.priciestprinting.com";
const DAILY_PLAY_STORAGE_KEY = "priciest-printing-daily-plays";
...
const ANSWER_CARD_FRAME_CLASS = "card-frame-answer";
const MAX_GUESS_COUNT = 15;
...
function getMaxPrintingPrice(printing: PrintingInfo): number {
  return Math.max(printing.prices.nonfoil ?? -1, printing.prices.foil ?? -1);
}
...
function renderCardFrame(card: ScryfallCard) {
  ...
  cardFrame.classList.remove(ANSWER_CARD_FRAME_CLASS);
...
function compareSetsByReleaseDate(a: SetInfo, b: SetInfo): number {
  if (a.releaseDate && b.releaseDate) {
    const dateComparison = a.releaseDate.localeCompare(b.releaseDate);
    if (dateComparison !== 0) {
...
function renderSetTimeline() {
  ...
  setTimeline.classList.remove("set-timeline-condensed");
  timelineSetIndexByCode.clear();

  const timelineSets = [...allSets].sort(compareSetsByReleaseDate);
...
function trackGuessedSet(setCode: string) {
  if (guessedSetCodesSeen.has(setCode)) {
    return;
  }
  guessedSetCodesSeen.add(setCode);
  guessedSetCodesInOrder.push(setCode);
}

function renderCondensedTimeline() {
  setTimeline.replaceChildren();
  setTimeline.classList.add("set-timeline-condensed");
  timelineSetIndexByCode.clear();

  const codesToRender = guessedSetCodesInOrder.length > 0
    ? guessedSetCodesInOrder
    : [...correctSetCodes];
  const condensedSets = codesToRender
    .map(setCode => allSets.find(item => item.code === setCode))
    .filter((set): set is SetInfo => set !== undefined)
    .sort(compareSetsByReleaseDate);

  condensedSets.forEach((set, index) => {
    const item = document.createElement("div");
    item.className = "timeline-set-item";
    item.dataset.setCode = set.code;
    const symbolLabel = getSetSymbolLabel(set);
    ...
    item.classList.add(correctSetCodes.has(set.code) ? "timeline-set-correct" : "timeline-set-incorrect");
    setTimeline.appendChild(item);
    timelineSetIndexByCode.set(set.code, index);
  });
...
function getSetMatchClass(guessedSetName: string): "result-set-partial-capitalized" | "result-set-partial-word" | "result-set-incorrect" {
  const correctName = correctPrinting?.setName ?? "";
  if (!correctName) return "result-set-incorrect";

  const correctWords = new Set(correctName.split(/\s+/).filter(Boolean));
  const guessedWords = guessedSetName.split(/\s+/).filter(Boolean);

  let hasCapitalizedMatch = false;
  let hasLowercaseMatch = false;

  for (const word of guessedWords) {
    if (!correctWords.has(word)) continue;
    const firstChar = word[0];
    if (firstChar && /^[A-Z]/.test(firstChar)) {
      hasCapitalizedMatch = true;
    } else {
      hasLowercaseMatch = true;
    }
  }

  if (hasCapitalizedMatch) return "result-set-partial-capitalized";
  if (hasLowercaseMatch) return "result-set-partial-word";
  return "result-set-incorrect";
}

function getFinishMatchLevel(guessedFinish: Finish, guessedStyle: PrintingStyle | null): "correct" | "partial" | "incorrect" {
  if (!correctPrinting) return "incorrect";
  const finishMatch = guessedFinish === correctFinish;
  if (!currentHardMode) {
    return finishMatch ? "correct" : "incorrect";
  }
  const styleMatch = guessedStyle === correctPrinting.style;
  if (finishMatch && styleMatch) return "correct";
  if (finishMatch || styleMatch) return "partial";
  return "incorrect";
}
...
function addGuessRow(set: SetInfo, finish: Finish, printing: PrintingInfo | null, guessedStyle: PrintingStyle | null = null) {
  ...
  let setMatchClass: string;
  let setShareEmoji: string;
  if (isSetCorrect) {
    setMatchClass = "result-set-correct";
    setShareEmoji = "🟩";
  } else {
    const partialClass = getSetMatchClass(set.name);
    setMatchClass = partialClass;
    if (partialClass === "result-set-partial-capitalized") setShareEmoji = "🟨";
    else if (partialClass === "result-set-partial-word") setShareEmoji = "🟧";
    else setShareEmoji = "🟥";
  }
  setCell.className = `result-set ${setMatchClass}`;
  appendSetNameNodes(setCell, set.iconSvgUri, set.name, set.code);

  const numberCell = document.createElement("div");
  const finishLevel = getFinishMatchLevel(finish, guessedStyle);
  let numberMatchClass: string;
  let numberShareEmoji: string;
  if (finishLevel === "correct") {
    numberMatchClass = "result-number-correct";
    numberShareEmoji = "🟩";
  } else if (finishLevel === "partial") {
    numberMatchClass = "result-number-partial";
    numberShareEmoji = "🟨";
  } else {
    numberMatchClass = "result-number-incorrect";
    numberShareEmoji = "🟥";
  }
  numberCell.className = `result-number ${numberMatchClass}`;
  ...
  let priceShareEmoji: string;
  if (printing === null) {
    priceCell.textContent = "";
    priceCell.style.backgroundColor = "#2a2a2a";
    priceShareEmoji = "⬛";
  } else {
    const price = getPrintingPrice(printing, finish);
    ...
    priceShareEmoji = getPriceShareEmoji(price, winningPrice);
  }
  ...
  shareRows.push(`${setShareEmoji}${numberShareEmoji}${priceShareEmoji}${yearEmoji}`);
...
function triggerVictoryEffect() {
  ...
  if (victoryEffectTimeoutId !== null) {
    window.clearTimeout(victoryEffectTimeoutId);
  }

  for (let i = 0; i < CONFETTI_PIECE_COUNT; i += 1) {
    const piece = document.createElement("span");
    piece.className = "victory-confetti-piece";
    const angle = (Math.PI * 2 * i) / CONFETTI_PIECE_COUNT;
    const distance = 90 + Math.random() * 180;
    piece.style.setProperty("--x", `${Math.cos(angle) * distance}px`);
    piece.style.setProperty("--y", `${Math.sin(angle) * distance}px`);
    piece.style.setProperty("--rot", `${90 + Math.random() * 360}deg`);
  ...
  victoryEffectTimeoutId = window.setTimeout(() => {
    victoryOverlay.classList.add("hidden");
    victoryConfetti.replaceChildren();
  }, VICTORY_EFFECT_DURATION_MS);
}

function renderAnswerCard(printing: PrintingInfo) {
  cardFrame.replaceChildren();
  cardFrame.classList.add(ANSWER_CARD_FRAME_CLASS);
  if (printing.imageUrl) {
    const image = document.createElement("img");
    image.className = "answer-card-image";
    image.src = printing.imageUrl;
    image.alt = `${selectedCardName} from ${printing.setName}`;
    cardFrame.appendChild(image);
    return;
  }
  const fallback = document.createElement("div");
  fallback.className = "answer-card-fallback";
  fallback.textContent = selectedCardName;
  cardFrame.appendChild(fallback);
}

function formatAllPrintingPrice(printing: PrintingInfo): string {
  const nonfoil = printing.prices.nonfoil !== null ? formatPrice(printing.prices.nonfoil) : "N/A";
  const foil = printing.prices.foil !== null ? formatPrice(printing.prices.foil) : "N/A";
  return `Non-foil: ${nonfoil} • Foil: ${foil}`;
}

function renderAllPrintings() {
  allPrintingsGrid.replaceChildren();
  const allPrintings = [...printingsBySet.values()]
    .flat()
    .sort((a, b) => {
      const aPrice = getMaxPrintingPrice(a);
      const bPrice = getMaxPrintingPrice(b);
      if (aPrice !== bPrice) {
        return bPrice - aPrice;
      }
      const setComparison = a.setName.localeCompare(b.setName);
      if (setComparison !== 0) {
  ...
  for (const printing of allPrintings) {
    ...
    card.className = "all-printing-item";

    if (printing.imageUrl) {
      const image = document.createElement("img");
      image.src = printing.imageUrl;
      image.alt = `${selectedCardName} from ${printing.setName} #${printing.collectorNumber}`;
      card.appendChild(image);
    } else {
      const fallback = document.createElement("div");
      fallback.className = "all-printing-item-fallback";
      fallback.textContent = selectedCardName;
      card.appendChild(fallback);
    }

    const caption = document.createElement("div");
    caption.className = "all-printing-caption";
    caption.textContent = `${printing.setName} #${printing.collectorNumber} • ${formatAllPrintingPrice(printing)}`;
    card.appendChild(caption);
    allPrintingsGrid.appendChild(card);
  }

  allPrintingsDetails.open = false;
  allPrintingsSection.classList.remove("hidden");
}
...
function revealVictory(
  ...
  finish: Finish,
  message?: string,
  options: { showCondensedTimeline?: boolean } = {},
) {
  ...
  renderAnswerCard(printing);
  renderAllPrintings();
  postWinActions.classList.remove("hidden");
  setGuessInput.disabled = true;
  finishGuessInput.disabled = true;
  styleGuessInput.disabled = true;
  setGuessButton.disabled = true;
  giveUpButton.disabled = true;

  if (!currentHardMode && options.showCondensedTimeline !== false) {
    renderCondensedTimeline();
  } else {
  ...
  if (message) {
    ...
    guessStatus.textContent = `Correct! ${selectedCardName}'s highest Scryfall price is the ${formatFinish(finish).toLowerCase()} printing in ${printing.setName} at ${formatPrice(winningPrice)}.`;
  }

  if (currentMode === "daily" && !message) {
    saveDailyPlayRecord(
    ...
    shareResultsButton.classList.toggle("hidden", currentMode !== "daily");
  }

  triggerVictoryEffect();
}

function showStoredDailyVictory(record: DailyPlayRecord) {
  const storedHardMode = record.hardMode ?? false;
  currentHardMode = storedHardMode;
  shareRows = [...record.shareRows];
  if (correctPrinting) {
    const message = record.cardName && record.priceText
      ? `You already completed today's daily. Today's card was ${record.cardName} at ${record.priceText}.`
      : "You already completed today's daily.";
    revealVictory(correctPrinting, correctFinish, message, { showCondensedTimeline: false });
  }
  shareResultsButton.classList.remove("hidden");
}
...
function revealOutOfGuesses() {
  if (guessedPrintingKeys.size < MAX_GUESS_COUNT || !correctPrinting) {
    return false;
  }
  ...
  revealVictory(
    correctPrinting,
    correctFinish,
    `Out of guesses. The answer was ${correctPrinting.setName} ${formatFinish(correctFinish).toLowerCase()}.`,
  );
  return true;
}
...
function submitGuess(guessedSet: SetInfo, guessedFinish: Finish, printing: PrintingInfo | null, guessedStyle: PrintingStyle | null = null) {
  ...
  trackGuessedSet(guessedSet.code);
  ...
  if (correctAnswerKeys.has(guessedKey) && printing) {
    ...
    revealVictory(printing, guessedFinish);
    return;
  }
  ...
  guessStatus.appendChild(
    document.createTextNode(
      ` ${formatFinish(guessedFinish).toLowerCase()} was not the highest-price printing. (${guessedPrintingKeys.size}/${MAX_GUESS_COUNT})`,
    ),
  );
}

function handleGiveUp() {
  if (!correctPrinting || setGuessButton.disabled) {
    return;
  }
  revealVictory(
    correctPrinting,
    correctFinish,
    `You gave up. The answer was ${correctPrinting.setName} ${formatFinish(correctFinish).toLowerCase()}.`,
  );
}
...
async function setupGame(mode: GameMode) {
  ...
  if (pendingStoredDailyRecord) {
    const storedRecord = pendingStoredDailyRecord;
    pendingStoredDailyRecord = null;
    showStoredDailyVictory(storedRecord);
  }
}
...
function resetGameState() {
  ...
  allPrintingsGrid.replaceChildren();
  allPrintingsSection.classList.add("hidden");
  allPrintingsDetails.open = false;
  guessStatus.textContent = "";
  setGuessInput.disabled = false;
  finishGuessInput.disabled = false;
  styleGuessInput.disabled = false;
  setGuessButton.disabled = false;
...
shareResultsButton.addEventListener("click", () => {
  ...
  const shareText = `Priciest Printing${modeLine}\n${shareRows.join("\n")}\n${SHARE_URL}`;
...
startDailyMode.addEventListener("click", () => {
  ...
  if (savedDailyRecord) {
    pendingStoredDailyRecord = savedDailyRecord;
